...
/* package */ final class Universes {
  ...
  /* package */ static <H, C> @NonNull Universe<H, C> compute(final @NonNull Universe<H, C> universe) {
    return (Universe<H, C>) Universes.UNIVERSES.computeIfAbsent(universe.id(), key -> universe);
  }

  /* package */ static <H, C> @Nullable Universe<H, C> get(final @NonNull String identifier) {
